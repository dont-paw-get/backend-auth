...
def upgrade() -> None:
    ...
    # PostgreSQL 은 IDENTITY 컬럼을 nullable 로 추가하는 것을 허용하지 않는다
    # (GENERATED ... AS IDENTITY 는 암묵적으로 NOT NULL 을 의미하므로, 여기에
    # 명시적 NULL 을 함께 쓰면 "conflicting NULL/NOT NULL declarations" 오류가
    # 난다). 그래서 먼저 평범한 nullable BIGINT 컬럼으로 추가해 값을 채우고
    # NOT NULL 로 바꾼 뒤, 마지막에 IDENTITY 속성을 붙인다.
    op.add_column(
        "member",
    ...
    op.execute("ALTER TABLE member ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY")
