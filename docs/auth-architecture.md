# 인증 아키텍처 명세: FE 주도 → BE 주도 전환

`backend-auth`의 Cognito 인증 구조에 대한 설계 결정·API 계약·오류 매핑·구현
현황을 기술한다. 원래 `PLAN.md` 한 파일에 계획과 명세가 함께 있었으나,
계획서에 명세가 섞이는 구조를 정리하며 분리했다.

**남은 할 일은 이 문서가 아니라 `PLAN.md`에 있다.** 절 번호는 분리 전과
동일하게 유지했다 — 본문의 상호 참조(§4.1, §8.2 등)를 깨지 않기 위해서다.
